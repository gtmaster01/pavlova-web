import { Check, X } from "lucide-react";
import { useTranslations } from "next-intl";
import { CourseDetailModal } from "./CourseDetailModal";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@/components/ui/card";

export function Pricing() {
    const t = useTranslations("Pricing");

    return (
        <section id="cenik" className="py-16 md:py-24 bg-white">
            <div className="container mx-auto px-4 md:px-6">
                <div className="text-center max-w-3xl mx-auto mb-16 space-y-4">
                    <h2 className="font-serif text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">
                        {t("title")}
                    </h2>
                    <p className="text-slate-600 text-lg">
                        {t("subtitle")}
                    </p>
                </div>

                <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                    {/* Basic Tier */}
                    <Card className="flex flex-col border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                        <CardHeader>
                            <CardTitle className="font-serif text-2xl text-slate-900">{t("basicTitle")}</CardTitle>
                            <CardDescription>{t("basicDescription")}</CardDescription>
                        </CardHeader>
                        <CardContent className="flex-1">
                            <div className="text-4xl font-bold text-slate-900 mb-6">{t("basicPrice")}</div>
                            <ul className="space-y-3">
                                <li className="flex items-center text-slate-600">
                                    <Check className="h-5 w-5 text-green-500 mr-2 flex-shrink-0" />
                                    {t("basicFeature1")}
                                </li>
                                <li className="flex items-center text-slate-600">
                                    <Check className="h-5 w-5 text-green-500 mr-2 flex-shrink-0" />
                                    {t("basicFeature2")}
                                </li>
                                <li className="flex items-center text-slate-600">
                                    <Check className="h-5 w-5 text-green-500 mr-2 flex-shrink-0" />
                                    {t("basicFeature3")}
                                </li>
                                <li className="flex items-center text-slate-400 decoration-slate-300">
                                    <X className="h-5 w-5 mr-2 flex-shrink-0" />
                                    <span className="line-through">{t("basicExcluded1")}</span>
                                </li>
                                <li className="flex items-center text-slate-400 decoration-slate-300">
                                    <X className="h-5 w-5 mr-2 flex-shrink-0" />
                                    <span className="line-through">{t("basicExcluded2")}</span>
                                </li>
                            </ul>
                        </CardContent>
                        <CardFooter>
                            <a
                                href="https://pavlova.kwiga.com/o/GMLw9epvreMU"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="block w-full"
                            >
                                <Button variant="outline" className="w-full border-primary text-primary hover:bg-primary/10 font-bold">
                                    {t("basicCta")}
                                </Button>
                            </a>
                        </CardFooter>
                    </Card>

                    {/* VIP Tier */}
                    <Card className="flex flex-col border-primary/50 shadow-lg relative overflow-hidden bg-slate-50/50">
                        <div className="absolute top-0 right-0 bg-primary text-slate-900 text-xs font-bold px-3 py-1 rounded-bl-lg">
                            {t("vipBadge")}
                        </div>
                        <CardHeader>
                            <CardTitle className="font-serif text-2xl text-slate-900">{t("vipTitle")}</CardTitle>
                            <CardDescription>{t("vipDescription")}</CardDescription>
                        </CardHeader>
                        <CardContent className="flex-1">
                            <div className="text-4xl font-bold text-slate-900 mb-6">{t("vipPrice")}</div>
                            <ul className="space-y-3">
                                <li className="flex items-center text-slate-700 font-medium">
                                    <Check className="h-5 w-5 text-primary mr-2 flex-shrink-0" />
                                    <strong>{t("vipFeature1")}</strong>
                                </li>
                                <li className="flex items-center text-slate-700 font-medium">
                                    <Check className="h-5 w-5 text-primary mr-2 flex-shrink-0" />
                                    {t("vipFeature2")}
                                </li>
                                <li className="flex items-center text-slate-700 font-medium">
                                    <Check className="h-5 w-5 text-primary mr-2 flex-shrink-0" />
                                    {t("vipFeature3")}
                                </li>
                                <li className="flex items-center text-slate-700 font-medium">
                                    <Check className="h-5 w-5 text-primary mr-2 flex-shrink-0" />
                                    {t("vipFeature4")}
                                </li>
                            </ul>
                        </CardContent>
                        <CardFooter>
                            <a
                                href="https://pavlova.kwiga.com/o/LahrVqDVsJO5"
                                target="_blank"
                                rel="noopener noreferrer"
                                className="block w-full"
                            >
                                <Button className="w-full bg-primary hover:bg-primary/90 text-slate-900 font-bold shadow-md hover:scale-105 hover:shadow-xl transition-all duration-200">
                                    {t("vipCta")}
                                </Button>
                            </a>
                        </CardFooter>
                    </Card>
                </div>
            </div>
            <div className="mt-12 text-center">
                <p className="text-slate-600 mb-4">{t("moreInfo")}</p>
                <div className="max-w-xs mx-auto">
                    <CourseDetailModal />
                </div>
            </div>
        </section>
    );
}
